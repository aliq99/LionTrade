...
def start_bot():
    ...
    try:
        ...
        return (
            True,
            f"Bot started successfully! (PID: {st.session_state.bot_process.pid})",
        )
...
with st.sidebar:
    ...
    if st.button("ðŸ’¾ Save Settings", type="primary", use_container_width=True):
        ...
        if strategy_choice == "Momentum":
            new_config.update({"ema_len": ema_len, "zscore_entry": zscore_entry})
        elif strategy_choice == "Scalping":
            new_config.update(
                {
        ...
        if save_config(new_config):
            st.success(f"âœ… {strategy_choice} settings saved!")
        else:
            st.error("âŒ Failed to save settings!")
    ...
    # Test Data Generator
    st.subheader("ðŸ§ª Test Live Data")
    ...
    if st.button("ðŸ“Š Generate Test Data", use_container_width=True):
        ...
        for i in range(num_points):
            ...
            timestamp = (datetime.now() - timedelta(minutes=num_points - i)).strftime(
                "%H:%M:%S"
            )
        ...
        try:
            ...
            st.error(f"âŒ Failed to generate test data: {e}")

    if st.button("ðŸ—‘ï¸ Clear Test Data", use_container_width=True):
        try:
            if LIVE_DATA_JSON.exists():
                os.remove(LIVE_DATA_JSON)
            st.success("âœ… Test data cleared!")
            time.sleep(1)
            st.rerun()
        except Exception as e:
            st.error(f"âŒ Failed to clear test data: {e}")
...
with tab1:
    ...
    with col_main:
        ...
        if not exits_df.empty:
            ...
            today_pnl = exits_df[exits_df["date"] == datetime.now().date()][
                "pnl_usdt"
            ].sum()
        else:
        ...
        m1.metric(
            "Total P&L",
            f"${total_pnl:.2f}",
            delta=f"+${today_pnl:.2f} today" if today_pnl != 0 else None,
        )
    ...
    with col_ai:
        ...
        if ai_status:
            ...
            if sentiment == "Bullish":
                st.success(f"**{sentiment}** ðŸ“ˆ")
            elif sentiment == "Bearish":
                st.error(f"**{sentiment}** ðŸ“‰")
            else:
                st.warning(f"**{sentiment}** âž¡ï¸")
            ...
            st.info("â³ Waiting for AI analysis...")
    ...
    with col_debug1:
        if LIVE_DATA_JSON.exists():
            ...
            st.caption(f"ðŸ“ File modified: {file_mod_time.strftime('%H:%M:%S')}")
            if time_diff > 60:
                st.caption(f"âš ï¸ File is {time_diff:.0f}s old!")
            else:
                st.caption(f"âœ… File is {time_diff:.0f}s old")
        else:
            st.caption("ðŸ“ live_data.json not found")

    with col_debug2:
        st.caption(f"ðŸ”„ Current time: {datetime.now().strftime('%H:%M:%S')}")
        st.caption(f"ðŸ”„ Refresh count: {st.session_state.get('refresh_count', 0)}")

    with col_debug3:
        if live_data:
            st.caption(f"ðŸ“Š Data keys: {list(live_data.keys())}")
            if live_data.get("test_data"):
                st.caption("ðŸ§ª Using test data")
            else:
                st.caption("ðŸ“ˆ Using live bot data")
    ...
    if live_data:
        ...
        if "prices" in live_data:
            ...
            st.info(
                "ðŸ“Š Showing single price point. Bot may need time to collect historical data."
            )
        ...
        if prices and len(prices) > 0:
            ...
            if len(prices) >= 2:
                ...
                price_change_pct = (
                    (price_change / prices[-2]) * 100 if prices[-2] != 0 else 0
                )

                col1, col2, col3 = st.columns(3)
                col1.metric("Current Price", f"${current_price:.4f}")
                col2.metric(
                    "Price Change", f"${price_change:.4f}", f"{price_change_pct:.2f}%"
                )
            ...
            st.warning("ðŸ“Š No price data found in live_data.json")
...
OR
{
  ...
  "timestamp": "2024-01-01 10:00:00"
        ...
        st.warning("ðŸ“Š No live price data available.")
        st.write("**Troubleshooting steps:**")
        st.write("1. âœ… Make sure the bot is running")
        st.write("2. ðŸ“ Check if `live_data.json` is being created")
        st.write("3. ðŸ”„ Verify the bot is updating the file regularly")
        st.write("4. ðŸ› Enable 'Debug Live Data' in sidebar to see data structure")
    ...
    if not trades_df.empty:
        ...
        if not exits_df.empty:
            ...
            with col2:
                ...
                exits_df["date"] = exits_df["ts_iso"].dt.date
                daily_stats = (
                    exits_df.groupby("date")
                    .agg({"pnl_usdt": ["count", "sum"]})
                    .reset_index()
                )
        ...
        st.info(
            "ðŸ“Š No performance data to display. Start trading to see analytics here."
        )
...
with tab3:
    ...
    if not trades_df.empty:
        ...
        st.dataframe(
            ...
            column_config={
                "ts_iso": st.column_config.DatetimeColumn(
        ...
        st.info("ðŸ“ No trade history available yet.")
...
with tab4:
    ...
    for file_name, exists in files_status.items():
        if exists:
            ...
            st.warning(f"âš ï¸ {file_name} - Not Found")
    ...
    st.write("**ðŸ“Š Live Data File Contents:**")
    if LIVE_DATA_JSON.exists():
        try:
            with open(LIVE_DATA_JSON, "r") as f:
